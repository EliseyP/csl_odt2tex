...
Макросы `\Troic` и `\Trisvjat` позволяют избежать недоработки кернинга для слов под титлом **Троица** и  **Трисвятое** в случае прописной первой буквы слова.   
Соответственно, слово Трⷪ҇ца (Троица) можно набрать макросом: `\Troic{а}`.  
