...
Причем, при отсутствии этих параметров их можно получить `user-defined` полей `odt`-документа (см. ниже). 
...
Использование:  
`csl2tex.py [-b] [-p] [-d] odt-filename [odt-filename]`  
...
При создании `PDF` нет возможности промежуточного редактирования `.tex` файла, геометрии и прочего. Скорее для предварительного просмотра.  
