...
Одгако при данной геометрии в оглавлении строка заголовка, которая берется из `\TITLE`, выдает badbox overfull. Поэтому в окончательном варианте вместо `\TOCENTRY` используется такая строка (вставлен разрыв `\\*`):   
\section[tocentry={\texorpdfstring{\KI Слꙋ́жба со а҆ка́ѳїстомъ сладча́йшемꙋ гдⷭ҇ꙋ на́шемꙋ і҆и҃сꙋ\\\\*хрⷭ҇тꙋ}{\TITLERU}},head={\TITLE}]{%  

Также добавлена вертикальная шпация (`СлужбаГосподу.tex:18`): 
`\VSPACE{-.7}{-.7}%`  
