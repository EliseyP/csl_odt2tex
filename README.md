...
При запуске с опцией `-I` файл `.tex` не генерируется автоматически из `odt`, в нем сохраняются изменения, сделанные вручную. Автоматически генерируется только `.init.tex`. В дальнейшем с помощью инструментов сравнивания (`meld`, `kdiff3` etc) изменения в исходном `odt`, которые оказались в `.init.tex`, можно внести в `.tex` для сборки `pdf`.   
...
Также есть расширение [Csl2PdfOxt](https://github.com/EliseyP/Csl2PdfOxt) для запуска этого диалога из `LibreOffice` для текущего открытого документа.
